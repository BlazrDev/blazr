...
namespace Blazr {
	...
	void Editor::renderTransformComponent(ImVec2 &cursorPos,
		...
		cursorPos.y += 20;
		ImGui::SetCursorPos(cursorPos);
		ImGui::InputFloat("##Rotation", &transform.rotation, 1.0f, 10.0f,
						  "%.1f");
		cursorPos.x -= 15;
		cursorPos.y += 35;
	...
	void Editor::renderIdentificationComponent(ImVec2 &cursorPos,
		...
		if (name != identification.name || groupName != identification.group) {
			std::copy(identification.name.begin(), identification.name.end(),
					  name);
		...
		if ((name != identification.name ||
			 groupName != identification.group) &&
			glfwGetKey(m_Window->GetWindow(), GLFW_KEY_ENTER) == GLFW_PRESS) {
			BLZR_CORE_INFO("Name changed from {0} to {1}", identification.name,
						   name);
			BLZR_CORE_INFO("Group changed from {0} to {1}",
	...
	void Editor::renderPhysicsComponent(ImVec2 &cursorPos,
		...
		ImGui::SetNextItemWidth(ImGui::GetContentRegionAvail().x);
		if (ImGui::BeginCombo("##TypesDropdown",
							  selectedTypeIndex == -1
								  ? "Choose a type"
		...
		ImGui::SetCursorPos(cursorPos);
		ImGui::SetNextItemWidth(ImGui::GetContentRegionAvail().x);
		ImGui::InputFloat("##density", &physics.GetAttributes().density, 0.1f,
						  1.0f, "%.1f");
		// friction
		cursorPos.x -= 90;
		cursorPos.y += 28;
		...
		ImGui::SetCursorPos(cursorPos);
		ImGui::SetNextItemWidth(ImGui::GetContentRegionAvail().x);
		ImGui::InputFloat("##gravityScale",
						  &physics.GetAttributes().gravityScale, 0.1f, 1.0f,
						  "%.1f");
		...
		ImGui::Checkbox("##isSensorCheckbox",
